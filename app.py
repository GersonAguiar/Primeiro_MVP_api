...
cliente_tag = Tag(name="Cliente", description="Adição, visualização e remoção de clientes com relação à base")
comentario_tag = Tag(name="Comentario", description="Adição de um comentário a clientes cadastrado na base")
...
def add_cliente(form: ClienteSchema):
    ...
    try:
        ...
        # adiciona cliente
        session.add(cliente)
        # realiza o camando de adição de novo cliente na tabela
        session.commit()
        logger.debug(f"Adicionado cliente de nome: '{cliente.nome}'")
        return apresenta_cliente(cliente), 200

    except IntegrityError as e:
        # duplicidade do nome é a provável razão do IntegrityError
        # na presente listagem a duplicidade do nome não é um problema
...
def get_cliente(query: ClienteBuscaSchema):
    ...
    session = Session()
    # faz a busca
    cliente = session.query(Cliente).filter(Cliente == cliente_id).first()
...
def del_cliente(query: ClienteBuscaSchema):
    ...
    if count:
        ...
        # se o cliente não foi encontrado
        error_msg = "Cliente não encontrado na base :/"
        logger.warning(f"Erro ao remover cliente #'{cliente_nome}', {error_msg}")
        return {"mesage": error_msg}, 404
...
def add_comentario(form: ComentarioSchema):
    ...
    # adiciona o comentário ao cliente
